...
func GetPlatforms(c *gin.Context) {
	...
	err = dbConn.Get(&objectTypeID, `SELECT id FROM object_types WHERE name = 'Платформа'`)
	if err != nil {
...
func GetPlatformByID(c *gin.Context) {
	...
	// Формат совместим с normalizePlatform на фронте
...
func CreatePlatform(c *gin.Context) {
	...
	// Используем ту же модель входа, что и для БС/приложений/технологий
...
func UpdatePlatform(c *gin.Context) {
	...
	for k, v := range input {
		a, ok := attrByName[k]
		if !ok {
			// неизвестный атрибут — пропускаем (или вернуть 400, по желанию)
			continue
		}

...
func GetApplications(c *gin.Context) {
	...
	if err := dbConn.Get(&objectTypeID, `SELECT id FROM object_types WHERE name = 'Приложение'`); err != nil {
...
func CreateApplication(c *gin.Context) {
	...
	// object_type "Приложение" — берём ID сами
	var appTypeID int
	if err := dbConn.Get(&appTypeID, `SELECT id FROM object_types WHERE name = 'Приложение'`); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Object type not found"})
		return
	}
	...
	for _, attr := range input.Attributes {
		...
		`, attr.AttributeID, appTypeID).Scan(&attrType, &isMultiple, pq.Array(&options), &dictName); err != nil {
			tx.Rollback()
		...
		if _, err := tx.Exec(`
			...
			VALUES ($1, $2, $3, $4)
		`, appTypeID, objectID, attr.AttributeID, attr.Value); err != nil {
			tx.Rollback()
...
func UpdateApplication(c *gin.Context) {
	...
	for _, p := range pairs {
		...
		if err := dbConn.QueryRowx(`
			...
			FROM attributes
			WHERE id = $1 AND object_type_id = $2
		`, p.ID, appTypeID).Scan(&attrType, &isMultiple, pq.Array(&options), &dictName); err != nil {
			tx.Rollback()
		...
		`, appTypeID, objectID, p.ID, p.Value); err != nil {
			tx.Rollback()

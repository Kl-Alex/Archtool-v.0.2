...
func GetInitiatives(c *gin.Context) {
	...
	if err := dbConn.Get(&objectTypeID, `SELECT id FROM object_types WHERE name = 'Инициатива'`); err != nil {
...
func GetInitiativeByID(c *gin.Context) {
	dbConn, err := db.Connect()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "DB error"})
		return
...
func CreateInitiative(c *gin.Context) {
	...
	var input models.CreateBusinessCapabilityInput // формат: { attributes:[{attribute_id,value}] }
	...
	var typeID int
	if err := dbConn.Get(&typeID, `SELECT id FROM object_types WHERE name = 'Инициатива'`); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Object type not found"})
		return
	}
	...
	for _, attr := range input.Attributes {
		...
		if _, err := tx.Exec(`
			...
			VALUES ($1, $2, $3, $4)
		`, typeID, objectID, attr.AttributeID, attr.Value); err != nil {
			tx.Rollback()
...
// PUT /api/initiatives/:id  (универсальный)
func UpdateInitiative(c *gin.Context) {
	...
	for _, p := range pairs {
		...
		if err := dbConn.QueryRowx(`
			...
			FROM attributes
			WHERE id = $1 AND object_type_id = $2
		`, p.ID, typeID).Scan(&attrType, &isMultiple, pq.Array(&options), &dictName); err != nil {
			tx.Rollback()
		...
		`, typeID, objectID, p.ID, p.Value); err != nil {
			tx.Rollback()

...
// Новые компоненты ниже
import ApplicationCapabilityForm from "../components/AppCapabilityForm";
import ApplicationCapabilityCard from "./AppCapabilityPassportPage";
...
const AppCapabilitiesRegistryPage = () => {
  ...
  const handleDelete = async (id) => {
    const confirmed = window.confirm("Удалить эту способность приложения?");
    if (!confirmed) return;
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <div className="text-xs text-lentaBlue">
                    Приложение: {highlightMatch(node.application_name || "—", query)} | Уровень: {node.level || "—"} | Владелец: {highlightMatch(node.owner || "", query)} | Домен: {highlightMatch(node.it_domain || "", query)}
                  </div>
                </div>
              </div>
              <div className="flex space-x-2 items-start invisible group-hover:visible">
                <button onClick={() => handleEdit(node)} className="text-gray-400 hover:text-lentaBlue" title="Изменить">
                  <Pencil size={18} />
                </button>
                <button onClick={() => setSelectedCard(node)} className="text-gray-400 hover:text-blue-500" title="Информация">
                  <Info size={18} />
                </button>
                <button onClick={() => handleDelete(node.id)} className="text-gray-400 hover:text-red-500" title="Удалить">
                  <Trash2 size={18} />
                </button>
              </div>
            </div>
          ...
          {open && hasChildren && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }}>
              {node.children.map((child) => (
                <TreeNode key={child.id} node={child} depth={depth + 1} query={query} highlightedId={highlightedId} />
              ))}
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <main className="flex-1 p-6 bg-lentaWhite overflow-auto">
        <div className="flex items-center justify-between mb-4 gap-2">
          ...
          <div className="flex gap-2">
            <button
              ...
              onClick={() => {
                const newMap = {};
                items.forEach((cap) => (newMap[cap.id] = !expandAll));
                setOpenMap(newMap);
            ...
            <button className="bg-lentaBlue text-white px-4 py-2 rounded hover:bg-blue-700" onClick={() => setShowCreateModal(true)}>
              Создать
            </button>
          </div>
        </div>
        ...
        <div className="mb-4 flex items-start flex-wrap sm:flex-nowrap gap-2">
          <div className="flex flex-col gap-2">
            <button onClick={() => setShowFilters((p) => !p)} className="flex items-center gap-1 text-lentaBlue border border-lentaBlue px-3 py-1 rounded hover:bg-lentaBlue hover:text-white">
              <Filter size={16} />
              Фильтры
            </button>
            ...
            {showFilters && (
              <div className="flex flex-wrap sm:flex-nowrap items-center gap-2">
                <button onClick={() => { setFilterLevel(""); setFilterOwner(""); setFilterDomain(""); setSearch(""); }} className="text-red-600 hover:text-red-800 transition shrink-0" title="Сбросить фильтры">
                  <XCircle size={20} />
                </button>

                <div className="flex-1 min-w-[150px]">
                  <LevelCombobox
                    levels={[...new Set(items.map((c) => c.level).filter(Boolean))]}
                ...
                <div className="flex-1 min-w-[150px]">
                  <OwnerCombobox
                    owners={[...new Set(items.map((c) => c.owner).filter(Boolean))]}
                ...
                <div className="flex-1 min-w-[150px]">
                  <DomainCombobox
                    domains={[...new Set(items.map((c) => c.it_domain).filter(Boolean))]}
        ...
        <div className="space-y-2">
          {loading ? (
            ...
            treeData.map((node) => (
              <TreeNode key={node.id} node={node} depth={0} query={search} highlightedId={highlightedId} />
            ))
          )}
        </div>
        ...
        {showCreateModal && (
          ...
          >
            <ApplicationCapabilityForm ref={createFormRef} onCreated={handleCreatedOrUpdated} notifyError={notifyError} />
          </CreateModal>
        )}
        ...
        {showEditModal && editingItem && (
          ...
          >
            <ApplicationCapabilityForm ref={editFormRef} existingData={editingItem} onCreated={handleCreatedOrUpdated} notifyError={notifyError} />
          </EditModal>
        )}
      </main>

      {selectedCard && (
        <ApplicationCapabilityCard capability={selectedCard} onClose={() => setSelectedCard(null)} onUpdated={handleCreatedOrUpdated} notifyError={notifyError} />
      )}
    </div>
  );
};
